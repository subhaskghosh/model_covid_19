...
\cite{Long2020}
	...
	\begin{table*}
	\centering
	\begin{tabularx}{\textwidth}[t]{p{0.09\textwidth}p{0.4\textwidth}p{0.25\textwidth}p{0.15\textwidth}}
		\hline
		...
		\hline
		$\kappa$ &  infectiousness factor asymptomatic &  & \\
		\hline
		$\omega$ &  infectiousness factor quarantined &  & \\
		\hline
		$\rho$ &  infectiousness factor isolated & & \\
		\hline
		$\sigma$ &  transition rate exposed to infectious & $1/14$-$1/3$ $day^{-1}$ \cite{Li489,doi:10.7326/M20-0504} & $1/5$ $day^{-1}$\\
		\hline
		$\alpha$ &  fraction of infections that become symptomatic & 0.5 days & \cite{Hu2020}\\
		\hline
		$\nu$ &  transition rate  asymptomatic to symptomatic & 0.5 days & \cite{Hu2020}\\
		\hline
		$\epsilon$ &  detection rate asymptomatic & 0.5 days & \cite{Hu2020}\\
		\hline
		$\varphi$ &  rate of asymptomatic developing symptoms & 0.5 days & \cite{Hu2020}\\
		\hline
		$\theta$ &  rate of detection of symptomatic & 0.5 days & \cite{Hu2020}\\
		\hline
		$\tau$ &  rate of developing life-threatening symptoms & 0.5 days & \cite{Hu2020}\\
		\hline
		$\lambda$ &  rate of developing life-threatening symptoms & 0.5 days & \cite{Hu2020}\\
		\hline
		$\gamma$ &  recovery rate of asymptomatic & 0.5 days & \cite{Hu2020}\\
		\hline
		$\eta$ &  recovery rate of symptomatic & 0.5 days & \cite{Hu2020}\\
		\hline
		$\mu$ &  recovery rate of quarantined & 0.5 days & \cite{Hu2020}\\
		\hline
		$\psi$ & recovery rate of isolated & 0.5 days & \cite{Hu2020}\\
		\hline
		$\zeta$ &  recovery rate of critical & 0.5 days & \cite{Hu2020}\\
		\hline
		$\delta$ &  mortality rate & 0.5 days & \cite{Hu2020}\\ [1ex] 
		\hline
	\end{tabularx}
...
The basic reproduction number is calculated for the special case when we have $\beta(t) = \beta_0, \epsilon(t) = \epsilon_0$, and $p = q = r = s = u = v = w = x = 0$. in following we explore the local stability of the disease-free equilibrium (DFE) using the next generation operator method \cite{Diekmann1990,VANDENDRIESSCHE200229}.
