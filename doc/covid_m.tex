...
%
%
We also define $\mathcal{X}_s$, as the set of all possible disease free states.
%
%
\begin{proposition}
For equation $\dot{X} = f(x) = \mathcal{F}(X) - \mathcal{V}(X)$, following holds:
\begin{enumerate}
...
\end{proposition}
\begin{proof}
Propositions (1) and (2), an be observed from the equations (\ref{eqn:baseline-compact-2}) and (\ref{eqn:baseline-compact-3}).  For proposition (3), 
\end{proof}
%
%
