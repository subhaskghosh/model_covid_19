...
parser.add_argument('--varphi', type=float, default=0.29, help='rate of quarantined to isolation')
parser.add_argument('--theta', type=float, default=0.45, help='rate of detection of symptomatic')
parser.add_argument('--tau', type=float, default=0.0002, help='rate of developing life-threatening symptoms in isolation')
parser.add_argument('--lamda', type=float, default=0.0002, help='rate of developing life-threatening symptoms for symptomatic')
parser.add_argument('--gamma', type=float, default=0.026, help='recovery rate of asymptomatic')
parser.add_argument('--eta', type=float, default=0.0571, help='recovery rate of symptomatic')
parser.add_argument('--mu', type=float, default=0.062, help='recovery rate of quarantined')
parser.add_argument('--psi', type=float, default=0.06, help='recovery rate of isolated')
parser.add_argument('--zeta', type=float, default=0.079, help='recovery rate of critical')
...
def Model(days, beta_0, t_0, beta_min, r, epsilon_0, s, epsilon_max, et_0, beta_new, u, t_1, beta_min_1, t_2, v):
    ...
    E_0 = 2000.0 / population
    I_0 = 1000.0 / population
    A_0 = 3000.0 / population
    Q_0 = 0.0003531609011034018
    H_0 = 0.0010797736585067262
    C_0 = 1.7854393007831365e-05
    D_0 = 7.0 / population
    R_0 = 1.0 / population
...
params_init_min_max = {"beta_0": (1.14, 0.9, 1.8),
                       "beta_min": (0.03, 0.00004, 0.54),
                       "t_0": (2, 2, 40),
                       "t_1": (55, 45, 200),
                       "et_0": (40, 2, 80),
                       "r": (0.03, 0.001, 0.1),
                       "epsilon_0": (0.171, 0.00001, 0.3),
                       "epsilon_max": (0.6,0.5,0.9999),
                       "s":(0.03, 0.001, 0.5),
                       "beta_new": (0.74, 0.005, 1.5),
                       "u": (0.03, 0.001, 0.1),
...
# Save plot data
x_mod_data = [(a+600) for a in x_data]
data = {'x_data': x_mod_data,
        'y_data': y_data,
        'best_fit': result.best_fit,
        'dely': dely}
confidence_plot_df = pd.DataFrame(data)
#confidence_plot_df.to_csv(f'../data/italy_confidence_plot_5.csv', index=False)
#
#
...
#plot_compare_df.to_csv(f'../data/italy_plot_compare_5.csv', index=False)
