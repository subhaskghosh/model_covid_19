...
def plot_compare(t,
    ...
    axes[1, 1].set_ylim(0, 2.5e-3)
    axes[1, 1].text(-0.1, 1.15, 'd', transform=axes[1, 1].transAxes, size=16, weight='bold')

    # 'Infected, No Symptoms: Model vs. Data'
    (markers, stemlines, baseline) = axes[2, 0].stem(t, quarantined[outbreak_shift:outbreak_shift + till_day],
                                                     label='Actual Recovered', linefmt='orange', basefmt=" ",
                                                     use_line_collection=True)
    plt.setp(markers, marker='o', markersize=6, markeredgecolor="orange", markeredgewidth=0.5,
             markerfacecolor=(0, 0, 0, 0.0))
...
def plot_evolution(t, I, A, Q, H, C, D, DR, TI, R, currently_infected, beta_over_time, epsilon_over_time):
    ...
    axes[0, 0].plot(t[:182], currently_infected, label='Current total infected: Data', color='#F29FD9')
    ...
    axes[0, 1].plot(t[:182], currently_infected, label='Current total infected: Data', color='#F29FD9',
                    marker='o', markersize=1, markeredgecolor="#F29FD9", markeredgewidth=0.5, markerfacecolor=(0, 0, 0, 0.0))
...
def plot_compare_india(t,
    ...
    axes[2].text(-0.1, 1.15, 'd', transform=axes[2].transAxes, size=16, weight='bold')
...
def plot_compare_brazil(t,
    ...
    axes[2].set_title('Deaths: Model vs. Data')
    axes[2].set_xlim(outbreak_shift, till_day)
    axes[2].set_ylim(0, 1.5e-2)
    axes[2].text(-0.1, 1.15, 'd', transform=axes[2].transAxes, size=16, weight='bold')

    fig.tight_layout()
    plt.show()

    # plt.savefig(f'../doc/Brazil_plot_model_data.pdf', dpi=600)
    # plt.clf()

def plot_evolution_Brazil(t, I, A, Q, H, C, D, DR, TI, R, currently_infected, beta_over_time, epsilon_over_time):
    short = 90
    long = 500
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    fig.subplots_adjust(hspace=0.4, wspace=0.4)
    ...
    # plt.savefig(f'../doc/Brazil_evolution.pdf', dpi=600)
    # plt.clf()

def plot_compare_spain(t,
                 TI, currently_infected,
                 R, recovered,
                 D, deaths,
                 outbreak_shift, till_day):
    ...
    axes[1].set_ylabel('Cases (fraction of the population)')
    axes[1].set_title('Recovered: Model vs. Data')
    axes[1].set_xlim(0, till_day)
    axes[1].set_ylim(0, 5e-3)
    axes[1].text(-0.1, 1.15, 'b', transform=axes[1].transAxes, size=16, weight='bold')
    ...
    axes[2].set_ylabel('Cases (fraction of the population)')
    axes[2].set_title('Deaths: Model vs. Data')
    axes[2].set_xlim(0, till_day)
    axes[2].set_ylim(0, 5e-3)
    axes[2].text(-0.1, 1.15, 'd', transform=axes[2].transAxes, size=16, weight='bold')

    fig.tight_layout()
    plt.show()

    # plt.savefig(f'../doc/Spain_plot_model_data.pdf', dpi=600)
    # plt.clf()

def plot_evolution_spain(t, I, A, Q, H, C, D, DR, TI, R, currently_infected, beta_over_time, epsilon_over_time):
    short = 90
    long = 200
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    fig.subplots_adjust(hspace=0.4, wspace=0.4)
    ...
    axes[0, 0].plot(t, I + A + Q + H + C + R, label='Cumulative infected', color='#E37332')
    axes[0, 0].plot(t, TI, label='Current total infected', color='#829FD9')
    axes[0, 0].plot(t[:179], currently_infected[35:], label='Current total infected: Data', color='#F29FD9')
    ...
    axes[0, 1].plot(t, I+A+Q+H+C+R, label='Cumulative infected', color='#E37332')
    axes[0, 1].plot(t, TI, label='Current total infected', color='#829FD9')
    axes[0, 1].plot(t[:179], currently_infected[35:], label='Current total infected: Data', color='#F29FD9')
