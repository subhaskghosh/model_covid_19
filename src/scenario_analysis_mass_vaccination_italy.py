...
import lmfit
import argparse
from src.plots import plot_scenario_vaccination_italy
...
def deriv(y, t, beta, kappa, omega, rho, sigma, alpha, nu, epsilon, varphi, theta, tau, lamda, gamma, eta, mu, psi, zeta, delta, phi, xi):
    E, I, A, Q, H, C, D, R, S, DR, V = y

    dEdt = beta(t) * (I + kappa * A + omega * Q + rho * H) * S + phi * beta(t) * (I + kappa * A + omega * Q + rho * H) * V - sigma * E
    ...
    dSdt = -beta(t) * (I + kappa * A + omega * Q + rho * H) * S - xi(t) * S
    dDRdt = ( mu * Q + psi * H + zeta * C )
    dVdt = xi(t) * S - phi * beta(t) * (I + kappa * A + omega * Q + rho * H) * V

    return dEdt, dIdt, dAdt, dQdt, dHdt, dCdt, dDdt, dRdt, dSdt, dDRdt, dVdt

def Model(days, beta_0, t_0, beta_min, r, epsilon_0, s, epsilon_max, et_0, beta_new, u, t_1, phi, xi_0, xi_max, vt_0, v):
    # Contact
    def beta(t):
        if t < t_0:
            return beta_0
    ...
    def xi(t):
        if t < vt_0:
            ...
            curr = xi_max - (xi_max - xi_0) * np.exp(-v * (t - vt_0))
            return curr
    ...
    DR_0 = 0.0
    V_0 = 0.0
    y0 = E_0, I_0, A_0, Q_0, H_0, C_0, D_0, R_0, S_0, DR_0, V_0

    t = np.linspace(0, days - 1, days)
    ret = odeint(deriv, y0, t, args=(
    beta, kappa, omega, rho, sigma, alpha, nu, epsilon, varphi, theta, tau, lamda, gamma, eta, mu, psi, zeta, delta, phi, xi))
    E, I, A, Q, H, C, D, R, S, DR, V = ret.T
    TI = Q + H + C

    beta_over_time = [beta(i) for i in range(len(t))]
    epsilon_over_time = [epsilon(i) for i in range(len(t))]
    xi_over_time = [xi(i) for i in range(len(t))]

    return t, E, I, A, Q, H, C, D, R, S, DR, TI, beta_over_time, epsilon_over_time, xi_over_time, V

outbreak_shift = 0
till_day = 75

best_values = {'beta_0': 1.8,
               't_0': 5.0,
               'beta_min': 0.04,
               'r': 0.05,
               'epsilon_0': 0.017,
...
phi_set = [1.0, 0.5, 0.25, 0.125, 0.0625]
xi_max_set = [0.5, 0.6, 0.7, 0.8, 0.9]

for xi_max in xi_max_set:
    simulation_values = {"phi":phi_set[0], "xi_0":0.0, "xi_max":xi_max, "vt_0":184+90, "v":0.011}

    t, E, I, A, Q, H, C, D, R, S, DR, TI, beta_over_time, epsilon_over_time, xi_over_time, V = Model(700,
                                                                                    best_values['beta_0'],
                                                                                    best_values['t_0'],
                                                                                    ...
                                                                                    best_values['u'],
                                                                                    best_values['t_1'],
                                                                                    simulation_values["phi"],
                                                                                    simulation_values["xi_0"],
                                                                                    simulation_values["xi_max"],
                                                                                    simulation_values["vt_0"],
                                                                                    simulation_values["v"])
    [TI1, DR1, R1, D1, CI1, CR1] = [TI, DR, R, D, (I + A + Q + H + C + R), C]

    simulation_values = {"phi": phi_set[1], "xi_0": 0.0, "xi_max": xi_max, "vt_0": 184 + 90, "v": 0.011}
    ...
    simulation_values = {"phi": phi_set[2], "xi_0": 0.0, "xi_max": xi_max, "vt_0": 184 + 90, "v": 0.011}
    t, E, I, A, Q, H, C, D, R, S, DR, TI, beta_over_time, epsilon_over_time, xi_over_time, V = Model(700,
                                                                                    best_values['beta_0'],
                                                                                    best_values['t_0'],
                                                                                    best_values['beta_min'],
    ...
    simulation_values = {"phi": phi_set[3], "xi_0": 0.0, "xi_max": xi_max, "vt_0": 184 + 90, "v": 0.011}
    t, E, I, A, Q, H, C, D, R, S, DR, TI, beta_over_time, epsilon_over_time, xi_over_time, V = Model(700,
                                                                                    best_values['beta_0'],
                                                                                    best_values['t_0'],
                                                                                    best_values['beta_min'],
    ...
    simulation_values = {"phi": phi_set[4], "xi_0": 0.0, "xi_max": xi_max, "vt_0": 184 + 90, "v": 0.011}
    t, E, I, A, Q, H, C, D, R, S, DR, TI, beta_over_time, epsilon_over_time, xi_over_time, V = Model(700,
                                                                                    best_values['beta_0'],
                                                                                    best_values['t_0'],
                                                                                    best_values['beta_min'],
    ...
    plot_scenario_vaccination_italy(t,
